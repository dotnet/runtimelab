        ...
        private static Dictionary<char, BDD> ComputeIgnoreCaseDictionary(CharSetSolver solver, CultureInfo culture)
        {
            ...
            try
            {
                ...
                for (uint i = 0; i <= 0xFFFF; i++)
                {
                    ...
                    if (RegularExpressions.Regex.IsMatch($"{cUpper}{cLower}", $"^(?i:{StringUtility.Escape(c, useNumericRepresentationOnly: true)}{StringUtility.Escape(c, useNumericRepresentationOnly: true)})$"))
                    {
