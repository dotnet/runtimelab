...
namespace System.Text.RegularExpressions.SRM.Unicode
{
    internal sealed class IgnoreCaseTransformer
    {
        ...
        {
            _solver = solver;
            _i_Invariant = solver.MkOr(_solver.MkCharConstraint('i'), solver.MkCharConstraint('I'));
            _i_Default = solver.MkOr(_i_Invariant, solver.MkCharConstraint(Turkish_I_WithDot));
            _i_Turkish = solver.MkOr(solver.MkCharConstraint('i'), solver.MkCharConstraint(Turkish_I_WithDot));
            _I_Turkish = solver.MkOr(solver.MkCharConstraint('I'), solver.MkCharConstraint(Turkish_i_WithoutDot));
        }
        ...
        private BDD GetIgnoreCaseRel(out BDD domain, string culture = null)
        {
            ...
            if (culture == string.Empty)
            {
                if (_ignoreCaseRel_Invariant == null)
                {
                    ...
                    _ignoreCaseRel_Invariant = _solver.MkAnd(inv_table, _solver.MkNot(_solver.ShiftLeft(tr_I_withdot_BDD, 16)));

                    // Remove Turkish_I_withDot from the domain of casesensitive characters in the default case
                    _ignoreCaseRel_Invariant_Domain = _solver.MkAnd(_ignoreCaseRel_Default_Domain, _solver.MkNot(tr_I_withdot_BDD));
                }
            ...
            if (IsTurkishAlphabet(culture))
            {
                ...
                if (_ignoreCaseRel_Turkish == null)
                {
                    ...
                    _ignoreCaseRel_Turkish = _solver.MkOr(tr_table, _solver.MkOr(i_trXi_tr, I_trXI_tr));
                    _ignoreCaseRel_Turkish_Domain = _solver.MkOr(_ignoreCaseRel_Default_Domain, tr_i_withoutdot_BDD);
                }
        ...
        public BDD Apply(char c, string culture = null)
        {
            ...
            switch (c)
            {
                ...
                case Turkish_I_WithDot:
                    return
                        culture == string.Empty ? _solver.MkCharConstraint(Turkish_I_WithDot) :
                        IsTurkishAlphabet(culture) ? _i_Turkish :
                        _i_Default;
                ...
                case Turkish_i_WithoutDot:
                    return
                        IsTurkishAlphabet(culture) ? _I_Turkish :
                        _solver.MkCharConstraint(Turkish_i_WithoutDot);

                case 'k':
                case 'K':
                case KelvinSign:
                    return _char_table_CI[c] = _solver.MkOr(_solver.MkOr(_solver.MkCharConstraint('k'), _solver.MkCharConstraint('K')), _solver.MkCharConstraint(KelvinSign));
                ...
                case <= '\x7F':
                    ...
                    return _char_table_CI[c] = _solver.MkOr(_solver.MkCharConstraint(char.ToLower(c)), _solver.MkCharConstraint(char.ToUpper(c)));
                ...
                default:
                    ...
                    return _char_table_CI[c] = Apply(_solver.MkCharConstraint(c));
            }
        }
        ...
        public BDD Apply(BDD bdd, string culture = null)
        {
            ...
            return _solver.MkOr(ignorecase, bdd);
        }
