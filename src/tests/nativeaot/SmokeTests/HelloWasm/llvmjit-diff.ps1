...
{
    Write-Host ""
    Write-Host "llvmjit-diff.ps1 - Find differences in code generated by NativeAOT-LLVM"
    Write-Host ""
    Write-Host " Options:"
    ...
    Write-Host " of the diffs as well as disassembly files under the ./llvmjit-diff/ directory, which can be"
...
if ($Analyze -or $Summary)
{
    ...
    else
    {
        function WasmObjDump($Options, $Path)
        {
            if ($OS -eq "wasi")
            {
                $UnbundlePath = "$([IO.Path]::GetDirectoryName($Path))/llvmjit-diff"
    ...
    if ($Diffs.Count -ne 0)
    {
        ...
        if ($Analyze)
        {
            $AnalysisDirectory = "$PSScriptRoot/llvmjit-diff"
