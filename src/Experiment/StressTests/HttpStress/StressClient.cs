    ...
    public class StressClient : IDisposable
    {
        ...
        private HttpClient CreateHttpClient()
        {
            HttpMessageHandler CreateHttpHandler()
            {
                ...
                if (_config.UseLlHttp)
                {
                    var sock = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                    sock.Connect(new DnsEndPoint(_baseAddress.Host, _baseAddress.Port));
                    var stream = new EnhancedNetworkStream(sock, true);

                    return new PrimitiveHttpHandler(new Http1Connection(stream, HttpPrimitiveVersion.Version11));
                }
        ...
        private async Task InitializeClient()
        {
            ...
            async Task SendTestRequestToServer(int maxRetries)
            {
                ...
                client.Timeout = Debugger.IsAttached? Timeout.InfiniteTimeSpan: TimeSpan.FromSeconds(5);
