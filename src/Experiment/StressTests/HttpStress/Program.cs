...
public static class Program
{
    ...
    private static bool TryParseCli(string[] args, [NotNullWhen(true)] out Configuration? config)
    {
        ...
        cmd.AddOption(new Option("-llHttp", "Use low level HTTP API for the stress client.") { Argument = new Argument<bool>("enable", false) });
        ...
        cmd.AddOption(new Option("-clientTimeout", "Default HttpClient timeout in seconds. Defaults to 60 seconds.") { Argument = new Argument<int>("seconds", Debugger.IsAttached? Timeout.Infinite: 60) });
        ...
        config = new Configuration()
        {
            ...
            UseLlHttp = cmdline.ValueForOption<bool>("-llHttp"),
    ...
    private static async Task<ExitCode> Run(Configuration config)
    {
        ...
        Console.WriteLine("         Handler: " + (config.UseWinHttpHandler ? "UseWinHttpHandler" : config.UseLlHttp ? "LL HTTP" : "SocketsHttpHandler"));
        Console.WriteLine();
