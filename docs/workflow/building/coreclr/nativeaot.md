...
- Build the Jits and the ILC: `build clr.jit+clr.wasmjit+clr.nativeaotlibs+clr.tools+clr.nativeaotruntime -c [Debug|Release]`. Note that `clr.jit` only needs to be built once.  Add the `libs` subsets if you want the packages for publishing, e.g. `build clr.jit+clr.wasmjit+clr.nativeaotlibs+clr.tools+libs -c Debug`.
